...
export function listDetail(fr) {
    ...
    for (let w of fr) {
        if (s) {
            console.log(`S: ${s} // fr_explication: ${fr_explication}`);
            ...
            console.log(`2 => S: ${s} // fr_explication: ${fr_explication}`);
            french = w.french;
...
export const decryptageData = (encryptedData) => {
    ...
    var bytes = CryptoJS.AES.decrypt(encryptedData, process.env.REACT_APP_KEY);
    return bytes.toString(CryptoJS.enc.Utf8);
};

export const readable = (array) => {
    let readableData = [];
    for (let a of array) {
        readableData.push(decryptageData(a));
    }
    return readableData;
};
